//! Backend→frontend event abstraction (Tauri-free core).
//!
...
//! operations) to the Tauri runtime. `EventSink` inverts that: core code
//! emits through the trait, and the command layer decides where events go —
//! the GUI (`TauriEventSink`, app-side — see `src-tauri/src/reporter.rs`,
//! since it depends on `tauri::AppHandle`), nowhere (`NullEventSink`,
//! headless), or a test buffer (`CollectingEventSink`).
//!
...
/// Buffers events for assertions in tests. Gated on the `test-util` feature
/// (in addition to `cfg(test)`) so dependent crates — the app crate's own
/// test builds — can use it too: `cfg(test)` here only covers this crate's
/// own unit tests, not app-side tests that pull this type in as a library
/// dependency.
