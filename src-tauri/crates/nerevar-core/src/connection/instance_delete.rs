...
/// shape as the mid-layer's `sync_client::sync::get_instance_sync_status`.
...
pub fn delete_instance(
    ...
    if let Some(role) = instance_is_running(process_manager, &instance_id)? {
        return Err(format!(
    ...
    clear_hosting_if_needed(sync_host, manifest_cache, &instance_id, &*sink)?;
